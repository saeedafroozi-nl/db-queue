...
class PollSettingsParser {
    ...
    private void fillSettings(PollSettings.Builder pollSettings, String name, String value) {
        try {
            switch (name) {
                case QueueConfigsReader.SETTING_NO_TASK_TIMEOUT -> pollSettings.withNoTaskTimeout(Duration.parse(value));
                case QueueConfigsReader.SETTING_BETWEEN_TASK_TIMEOUT -> pollSettings.withBetweenTaskTimeout(Duration.parse(value));
                case QueueConfigsReader.SETTING_FATAL_CRASH_TIMEOUT -> pollSettings.withFatalCrashTimeout(Duration.parse(value));
                case QueueConfigsReader.SETTING_BATCH_SIZE -> pollSettings.withBatchSize(Integer.parseInt(value));
                case QueueConfigsReader.SETTING_QUERY_VERSION -> pollSettings.withQueryVersion(Integer.parseInt(value));
                default -> {
                }
            }
        } catch (RuntimeException exc) {
