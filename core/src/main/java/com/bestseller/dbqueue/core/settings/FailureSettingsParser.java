...
class FailureSettingsParser {
    ...
    private void fillSettings(FailureSettings.Builder failureSettings, String name, String value) {
        try {
            switch (name) {
                case QueueConfigsReader.SETTING_RETRY_TYPE -> failureSettings.withRetryType(parseRetryType(value));
                case QueueConfigsReader.SETTING_RETRY_INTERVAL -> failureSettings.withRetryInterval(Duration.parse(value));
                default -> {
                }
            }
        } catch (RuntimeException exc) {
    ...
    private static FailRetryType parseRetryType(String name) {
        return switch (name) {
            case QueueConfigsReader.VALUE_TASK_RETRY_TYPE_GEOMETRIC -> FailRetryType.GEOMETRIC_BACKOFF;
            case QueueConfigsReader.VALUE_TASK_RETRY_TYPE_ARITHMETIC -> FailRetryType.ARITHMETIC_BACKOFF;
            case QueueConfigsReader.VALUE_TASK_RETRY_TYPE_LINEAR -> FailRetryType.LINEAR_BACKOFF;
