...
public class ProcessingSettingsParser {
    ...
    private void fillSettings(ProcessingSettings.Builder processingSettings, String name, String value) {
        try {
            switch (name) {
                case QueueConfigsReader.SETTING_THREAD_COUNT -> processingSettings.withThreadCount(Integer.valueOf(value));
                case QueueConfigsReader.SETTING_PROCESSING_MODE -> processingSettings.withProcessingMode(parseProcessingMode(value));
                default -> {
                }
            }
        } catch (RuntimeException exc) {
    ...
    private static ProcessingMode parseProcessingMode(String name) {
        return switch (name) {
            case QueueConfigsReader.VALUE_PROCESSING_MODE_SEPARATE_TRANSACTIONS -> ProcessingMode.SEPARATE_TRANSACTIONS;
            case QueueConfigsReader.VALUE_PROCESSING_MODE_WRAP_IN_TRANSACTION -> ProcessingMode.WRAP_IN_TRANSACTION;
            case QueueConfigsReader.VALUE_PROCESSING_MODE_USE_EXTERNAL_EXECUTOR -> ProcessingMode.USE_EXTERNAL_EXECUTOR;
